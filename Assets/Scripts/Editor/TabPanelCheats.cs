...
public class TabPanelCheats : EditorWindow{
    ...
    void ClearResources(){
        PlayerScript.Instance.ClearAllResource();
    }
    void Add500kGold(){
        if(PlayerScript.Instance != null) PlayerScript.Instance.AddResource(new Resource(TypeResource.Gold, 500, 3));
    }
    void AddSimpleHire(){
    	if(PlayerScript.Instance != null) PlayerScript.Instance.AddResource(new Resource(TypeResource.SimpleHireCard, 50, 0));
    }
    void AddSimpleTask(){
        if(PlayerScript.Instance != null) PlayerScript.Instance.AddResource(new Resource(TypeResource.SimpleTask, 50, 0));
    }
    void AddSpecialTask(){
        if(PlayerScript.Instance != null) PlayerScript.Instance.AddResource(new Resource(TypeResource.SpecialTask, 50, 0));
    }
    void AddRaceHire(){
        if(PlayerScript.Instance != null) PlayerScript.Instance.AddResource(new Resource(TypeResource.RaceHireCard, 50, 0));
    }
    void AddSpinCoin(){
     ...
     void AddItem(){
        if(IDItem != 0){
            if(InventoryControllerScript.Instance != null){
                ...
                if(item != null){
                    InventoryControllerScript.Instance.AddItem(new ItemController(item, 1));
                }else{
                    Debug.Log("хуйня");
                }
            }
