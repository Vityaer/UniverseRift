...
public partial class HeroControllerScript : MonoBehaviour
{
	...
	protected virtual void FindAvailableCells(){myPlace.StartCheckMove(hero.characts.baseCharacteristic.Speed, this, playerCanController: !AIController.Instance.CheckMeOnSubmission(side));}
...
//Attack
	...
	public Coroutine GetRetaliation(HeroControllerScript attackedHero){
		return StartCoroutine(IGetRetaliation(attackedHero));
	}
	protected IEnumerator IGetRetaliation(HeroControllerScript attackedHero){
		while (flagAnimFinish == false){yield return null; }
